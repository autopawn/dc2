...
problem *problem_init(int n_facs, int n_clis){
    ...
    prob->firstr_per_size_n_sols = safe_malloc(sizeof(int)*(prob->n_facs+2));
    memset(prob->firstr_per_size_n_sols,0,     sizeof(int)*(prob->n_facs+2));
    prob->firstr_per_size_n_sols_after_red = safe_malloc(sizeof(int)*(prob->n_facs+2));
    memset(prob->firstr_per_size_n_sols_after_red,0,     sizeof(int)*(prob->n_facs+2));
    prob->firstr_per_size_n_local_optima = safe_malloc(sizeof(int)*(prob->n_facs+2));
    memset(prob->firstr_per_size_n_local_optima,0,     sizeof(int)*(prob->n_facs+2));
    //
    return prob;
}
...
void problem_free(problem *prob){
    ...
    free(prob->firstr_per_size_n_sols);
    free(prob->firstr_per_size_n_sols_after_red);
    free(prob->firstr_per_size_n_local_optima);
    // Free problem
    free(prob);
}
